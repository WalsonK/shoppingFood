...
exports.updateLowQuant = async(req, res) =>{
    const houseId = await Users.getHouse(req.body.id);
    const lowQuant = await Users.getHouseLowQuant(houseId);
    if(req.body.lowQuant != lowQuant){
        const isUpdated = await Users.setHouseLowQuant(req.body.lowQuant, houseId);
        if(isUpdated != 0){
            res.status(200).json({ error: false, message: 'Quantité minimal mis à jour !' });
        }else{
